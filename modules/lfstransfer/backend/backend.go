...
var _ transfer.Backend = (*ForgejoBackend)(nil)

// ForgejoBackend is an adapter between git-lfs-transfer library and Forgejo's internal LFS API
...
func New(ctx context.Context, repo, op, token string, logger transfer.Logger) (transfer.Backend, error) {
	...
	return &ForgejoBackend{ctx: ctx, server: server, op: op, authToken: token, logger: logger}, nil
}

// Batch implements transfer.Backend
func (g *ForgejoBackend) Batch(_ string, pointers []transfer.BatchItem, args transfer.Args) ([]transfer.BatchItem, error) {
...
func (g *ForgejoBackend) Download(oid string, args transfer.Args) (_ io.ReadCloser, _ int64, retErr error) {
	...
	// X-Forgejo-LFS-Content-Length is set to avoid being affected by reverse proxy or compression
	respSize, err := strconv.ParseInt(resp.Header.Get("X-Forgejo-LFS-Content-Length"), 10, 64)
	if err != nil {
...
func (g *ForgejoBackend) Upload(oid string, size int64, r io.Reader, args transfer.Args) error {
	idMapStr, exists := args[argID]
	if !exists {
		return ErrMissingID
	}
...
func (g *ForgejoBackend) Verify(oid string, size int64, args transfer.Args) (transfer.Status, error) {
...
func (g *ForgejoBackend) LockBackend(_ transfer.Args) transfer.LockBackend {
	return newForgejoLockBackend(g)
}
