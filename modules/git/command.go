...
// EscapePathspec escapes a file path for use as a Git pathspec argument.
// Handles special characters that have meaning in pathspec or shell contexts.
func EscapePathspec(path string) string {
	needsLiteral := strings.ContainsAny(path, "*?[]")

	needsQuoting := strings.ContainsAny(path, " \"\\") || strings.Contains(path, "'")

	if !needsLiteral && !needsQuoting {
		return path
	}
...
// EscapePathspecs escapes an array of paths for use as Git pathspec arguments
func EscapePathspecs(paths []string) []string {
	result := make([]string, len(paths))
	for i, path := range paths {
		result[i] = EscapePathspec(path)
	}
	return result
}
