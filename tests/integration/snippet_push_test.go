...
func TestSnippetsPushSSH(t *testing.T) {
	onApplicationRun(t, func(t *testing.T, u *url.URL) {
		withKeyFile(t, "test-snippet-key", func(keyFile string) {
			ctx := NewAPITestContext(t, "user2", "", auth_model.AccessTokenScopeWriteUser)

			t.Run("CreateUserKey", doAPICreateUserKey(ctx, "test-key-snippet", keyFile))

			tempDir := t.TempDir()

			require.NoError(t, git.Clone(db.DefaultContext, createSSHUrl("snippets/df852aec.git", u).String(), tempDir, git.CloneRepoOptions{}))
