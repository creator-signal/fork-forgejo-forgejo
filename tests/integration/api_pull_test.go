...
func TestAPIPullMergeMessage(t *testing.T) {
	defer tests.PrepareTestEnv(t)()

	repo := unittest.AssertExistsAndLoadBean(t, &repo_model.Repository{ID: 1})
	ctx := NewAPITestContext(t, "user2", repo.Name, auth_model.AccessTokenScopeReadRepository)

	// Test getting merge message for an existing pull request
	req := NewRequestf(t, "GET", "/api/v1/repos/%s/%s/pulls/2/merge-message", repo.OwnerName, repo.Name).AddTokenAuth(ctx.Token)
	resp := ctx.Session.MakeRequest(t, req, http.StatusOK)

	var mergeMessage api.PullRequestMergeMessage
	DecodeJSON(t, resp, &mergeMessage)

	// Verify the merge message structure
	assert.NotEmpty(t, mergeMessage.Title)
	assert.Contains(t, mergeMessage.Title, "pull request")
	assert.NotEmpty(t, mergeMessage.Body)
	assert.Contains(t, mergeMessage.Body, "Reviewed-on:")

	// Test with different merge styles
	styles := []string{"merge", "rebase", "rebase-merge", "squash"}
	for _, style := range styles {
		t.Run(fmt.Sprintf("style_%s", style), func(t *testing.T) {
			req := NewRequestf(t, "GET", "/api/v1/repos/%s/%s/pulls/2/merge-message?style=%s", repo.OwnerName, repo.Name, style).AddTokenAuth(ctx.Token)
			resp := ctx.Session.MakeRequest(t, req, http.StatusOK)

			var mergeMessage api.PullRequestMergeMessage
			DecodeJSON(t, resp, &mergeMessage)

			// For rebase style, the message might be empty
			if style == "rebase" {
				// Rebase style may return empty message
			} else {
				assert.NotEmpty(t, mergeMessage.Title)
			}
		})
	...
	// Test with invalid merge style
	req = NewRequestf(t, "GET", "/api/v1/repos/%s/%s/pulls/2/merge-message?style=invalid", repo.OwnerName, repo.Name).AddTokenAuth(ctx.Token)
	ctx.Session.MakeRequest(t, req, http.StatusBadRequest)

	// Test with non-existent pull request
	req = NewRequestf(t, "GET", "/api/v1/repos/%s/%s/pulls/9999/merge-message", repo.OwnerName, repo.Name).AddTokenAuth(ctx.Token)
	ctx.Session.MakeRequest(t, req, http.StatusNotFound)

	// Test without proper permissions - should return 404 (not found instead of unauthorized for privacy)
	ctx = NewAPITestContext(t, "user1", repo.Name, auth_model.AccessTokenScopeReadUser)
	req = NewRequestf(t, "GET", "/api/v1/repos/%s/%s/pulls/2/merge-message", repo.OwnerName, repo.Name).AddTokenAuth(ctx.Token)
	ctx.Session.MakeRequest(t, req, http.StatusNotFound)
}
