  ...
  test:
    steps:
      - run: echo OK`
		...
		doAPIEditRepository(testContext, &api.EditRepoOption{Archived: util.ToPointer(true)})(t)

		unittest.AssertCount(t, &actions_model.ActionSchedule{RepoID: repo.ID}, 0)
		unittest.AssertCount(t, &actions_model.ActionScheduleSpec{RepoID: repo.ID}, 0)

		doAPIEditRepository(testContext, &api.EditRepoOption{Archived: util.ToPointer(false)})(t)
