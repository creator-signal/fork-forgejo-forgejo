...
test('Combo Markdown: preview mode switch', async ({page}) => {
  ...
  expect(await page.locator('.markdown-toolbar-switch .switch').evaluate((el) => getComputedStyle(el).height)).toBe(await page.locator('md-header.markdown-toolbar-button').evaluate((el) => getComputedStyle(el).height));
