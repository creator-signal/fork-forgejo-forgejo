...
test.describe('Releases', () => {
  test('External Release Attachments', async ({page, isMobile}, workerInfo) => {
    ...
    // Click "New Release"
    await page.goto('/user2/rep%C3%B22/releases');
    await page.click('.button.small.primary');

    // Fill out form and create new release
    await expect(page).toHaveURL('/user2/rep%C3%B22/releases/new');
    ...
    await expect(page).toHaveURL('/user2/rep%C3%B22/releases');
    ...
    await expect(page.locator('.download[open] li:nth-of-type(1) a')).toHaveAttribute('href', '/user2/rep%C3%B22/archive/2.0.zip');
    ...
    await expect(page.locator('.download[open] li:nth-of-type(2) a')).toHaveAttribute('href', '/user2/rep%C3%B22/archive/2.0.tar.gz');
    ...
    await expect(page).toHaveURL('/user2/rep%C3%B22/releases/edit/2.0');
    await validate_form({page}, 'fieldset');
  ...
  test('Release name equals tag name if created from tag', async ({page}) => {
    await page.goto('/user2/rep%C3%B22/releases/new?tag=v1.1');
  ...
  test.afterEach(async ({page}) => {
    // Delete release
    const response = await page.goto('/user2/rep%C3%B22/releases/edit/2.0');
