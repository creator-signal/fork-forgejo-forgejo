...
test('multi-commit commenting', async ({page, request}) => {
  ...
  try {
    ...
    // FIXME: here's the first assertion that is broken in this test -- disabled so that the next test step can continue
    ...
    await expect(issuesResp.ok()).toBeTruthy();
    ...
    for (const issue of issues) {
      ...
      await expect(delResp.ok()).toBeTruthy();
    }
  }
});
