...
RENOVATE_NPM_PACKAGE ?= renovate@44.31.0 # renovate: datasource=docker packageName=data.forgejo.org/renovate/renovate
...
coverage-run-migration-%: generate-ini-% | compute-migration-packages
	...
	for pkg in $(MIGRATION_PACKAGES); do \
		$(MAKE) PROJECT_ROOT="$(CURDIR)" PROJECT_CONF=tests/$*.ini COVERAGE_TEST_DATABASE=$* COVERAGE_TEST_ARGS= COVERAGE_TEST_PACKAGES=$$pkg coverage-run ; \
	done

coverage-run-integration-%: generate-ini-%
	$(MAKE) PROJECT_ROOT="$(CURDIR)" PROJECT_CONF=tests/$*.ini COVERAGE_TEST_DATABASE=$* COVERAGE_TEST_PACKAGES=forgejo.org/tests/integration coverage-run

coverage-run-e2e: frontend playwright generate-ini-sqlite
	$(MAKE) PROJECT_ROOT="$(CURDIR)" PROJECT_CONF=tests/sqlite.ini COVERAGE_TEST_DATABASE=sqlite PLAYWRIGHT_PROJECT=firefox COVERAGE_TEST_PACKAGES=forgejo.org/tests/e2e COVERAGE_TEST_ARGS="-test.run TestE2e ${COVERAGE_TEST_ARGS}" coverage-run
