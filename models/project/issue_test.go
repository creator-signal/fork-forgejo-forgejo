...
func TestAddIssueToProject(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		err := AddIssueToProject(db.DefaultContext, 1, 4, 1, 0)
		require.NoError(t, err)
	...
	t.Run("ErrorDuplicate", func(t *testing.T) {
		...
		err := AddIssueToProject(db.DefaultContext, 1, 1, 1, 0)
		require.Error(t, err)
	...
	t.Run("ErrorColumnNotInProject", func(t *testing.T) {
		...
		err := AddIssueToProject(db.DefaultContext, 1, 6, 5, 0)
		require.Error(t, err)
		assert.True(t, IsErrProjectColumnNotExist(err))
	})

	t.Run("AutoSorting", func(t *testing.T) {
		// When sorting is 0, it should auto-assign to end
		err := AddIssueToProject(db.DefaultContext, 1, 7, 1, 0)
		require.NoError(t, err)

		card, err := GetProjectCard(db.DefaultContext, 1, 7)
		require.NoError(t, err)
		// Should have a sorting value assigned
...
func TestMoveCardToColumn(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		err := MoveCardToColumn(db.DefaultContext, card.ID, 2, 0)
		require.NoError(t, err)
	...
	t.Run("ErrorCardNotFound", func(t *testing.T) {
		err := MoveCardToColumn(db.DefaultContext, 99999, 2, 0)
		require.Error(t, err)
	...
	t.Run("ErrorColumnNotInSameProject", func(t *testing.T) {
		...
		err := MoveCardToColumn(db.DefaultContext, card.ID, 5, 0)
		require.Error(t, err)
		assert.True(t, IsErrProjectColumnNotExist(err))
	})
}
...
func TestMoveCardToColumnWithPositionShift(t *testing.T) {
	...
	t.Run("MoveToNewColumnWithPosition", func(t *testing.T) {
		...
		require.NoError(t, AddIssueToProject(db.DefaultContext, projectID, 22, sourceColumn, 0))
