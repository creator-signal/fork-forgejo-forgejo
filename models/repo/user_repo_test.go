...
func TestGetUserStarCount(t *testing.T) {
	...
	require.NoError(t, err)
	require.EqualValues(t, 5, count, "owner should see all their own stars")
	count, err = repo_model.GetUserStarCount(t.Context(), profileUser, nil)
	t.Logf("anonymous view: count=%d err=%v", count, err)
	require.NoError(t, err)
	require.EqualValues(t, 2, count, "anon should only see public repos with public owners")
	...
	require.NoError(t, err)
	require.EqualValues(t, 2, count, "unrelated signed-in user should only see public repos")
	...
	require.NoError(t, err)
	require.EqualValues(t, 3, count, "collaborator should see public repos + their collab private repo")
	...
	require.NoError(t, err)
	require.EqualValues(t, 3, count, "org member should see public repos + org's private repo")
