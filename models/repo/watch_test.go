...
func TestUnwatchRepos(t *testing.T) {
	...
	assert.EqualValues(t, 1, unittest.GetCount(t, &repo_model.Watch{UserID: 4, RepoID: 1}, "source = ? AND watch_selection_issues = false AND watch_selection_pull_requests = false AND watch_selection_releases = false", repo_model.WatchSourceExplicit))
	assert.EqualValues(t, 1, unittest.GetCount(t, &repo_model.Watch{UserID: 4, RepoID: 2}, "source = ? AND watch_selection_issues = false AND watch_selection_pull_requests = false AND watch_selection_releases = false", repo_model.WatchSourceExplicit))
}
