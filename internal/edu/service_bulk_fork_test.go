...
func TestBulkForkForAssignment_ReturnsPending(t *testing.T) {
...
func TestExecuteBulkFork_AllStudents(t *testing.T) {
	mockRepo := new(MockRepository)
	mockForker := new(MockRepoForker)
	mockUsers := new(MockUserCreator)
	...
	task := &BulkForkTask{ID: 1, AssignmentID: 1, TotalUsers: 2, Status: StatusPending}
	...
	svc.executeBulkFork(context.Background(), task, 1, 50, baseRepo, enrollments)
...
func TestExecuteBulkFork_SkipsExistingSubmissions(t *testing.T) {
	mockRepo := new(MockRepository)
	mockForker := new(MockRepoForker)
	mockUsers := new(MockUserCreator)
	svc := NewService(mockRepo, mockForker, mockUsers).(*service)
	...
	task := &BulkForkTask{ID: 1, AssignmentID: 1, TotalUsers: 1, Status: StatusPending}
...
func TestExecuteBulkFork_ForkError(t *testing.T) {
	mockRepo := new(MockRepository)
	mockForker := new(MockRepoForker)
	mockUsers := new(MockUserCreator)
	svc := NewService(mockRepo, mockForker, mockUsers).(*service)
