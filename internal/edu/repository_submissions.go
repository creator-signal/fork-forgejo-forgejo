...
func (r *dbRepository) GetSubmission(ctx context.Context, assignmentID, userID int64) (*Submission, error) {
	...
	query, args, err := psql.Select("id", "assignment_id", "user_id", "student_repo_id", "status", "created_unix", "updated_unix").
	...
	var s Submission
	err = row.Scan(&s.ID, &s.AssignmentID, &s.UserID, &s.StudentRepoID, &s.Status, &s.CreatedUnix, &s.UpdatedUnix)
	if err != nil {
