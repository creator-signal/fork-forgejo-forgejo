...
for (const [key, value] of Object.entries(mergeData)) {
  ...
  if (key.startsWith('mergeForm')) {
    ...
    const finalKey = stripped.charAt(0).toLowerCase() + stripped.slice(1);
...
for (const [key, value] of Object.entries(mergeData)) {
  ...
  if (key.startsWith('mergeStyles')) {
    ...
    const match = stripped.match(
      /^(.+?)(Allowed|TextDoMerge|MergeTitleFieldText|HideMergeMessageTexts|MergeMessageFieldText|HideAutoMerge)$/,
    );
