...
func TestReorderCardsInColumn(t *testing.T) {
	...
	t.Run("Normal reordering", func(t *testing.T) {
		cardPositions := []CardPosition{
			{IssueID: issue2.ID, Sorting: 0}, // Move issue2 to position 0
			{IssueID: issue1.ID, Sorting: 1}, // Move issue1 to position 1
		}
		...
		cards, err := project_model.GetProjectCardsInColumn(db.DefaultContext, column.ID, db.ListOptions{})
		require.NoError(t, err)
		require.Len(t, cards, 2)

		// Cards should be ordered by sorting value
	...
	t.Run("Non-existent issue", func(t *testing.T) {
		cardPositions := []CardPosition{
			{IssueID: 99999, Sorting: 0}, // Non-existent issue
		}

		err := ReorderCardsInColumn(db.DefaultContext, column, cardPositions)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "some cards do not exist")
	})

	t.Run("Issue from wrong repository", func(t *testing.T) {
		// Create another repository and issue
...
func TestReorderCardsInColumnConcurrent(t *testing.T) {
	...
	// Run concurrent reordering operations
	...
	for i := 0; i < 3; i++ {
		...
		go func(workerID int) {
			...
			// Each worker tries to reorder cards differently
			cardPositions := make([]CardPosition, len(issues))
			for j, issue := range issues {
				// Different workers use different sorting orders
			...
			err := ReorderCardsInColumn(context.Background(), column, cardPositions)
			if err != nil {
				errorsMutex.Lock()
	...
	// All operations should complete successfully due to transaction safety
	...
	// Verify no duplicate sorting values
...
func TestRemoveCardFromProject(t *testing.T) {
	...
	t.Run("Successfully remove card from project", func(t *testing.T) {
		...
		require.NoError(t, project_model.AddIssueToProject(db.DefaultContext, project.ID, issue.ID, column.ID, 0))
	...
	t.Run("No error when removing non-existent card", func(t *testing.T) {
		// Try to remove an issue that's not in the project
		// The underlying function may not return an error for non-existent cards
		err := RemoveCardFromProject(db.DefaultContext, project, int64(99999))
		// This is acceptable behavior - either no error or an error is fine
		_ = err
	})

	t.Run("No error when removing already removed card", func(t *testing.T) {
		// Try to remove the same issue again (already removed above)
		err := RemoveCardFromProject(db.DefaultContext, project, issue.ID)
		// The underlying function might not return an error for non-existent cards
