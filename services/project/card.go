...
func ReorderCardsInColumn(ctx context.Context, column *project_model.Column, cardPositions []CardPosition) error {
	...
	// Validate no duplicate sorting positions
	seen := make(map[int64]bool, len(cardPositions))
	for _, cp := range cardPositions {
		if seen[cp.Sorting] {
			return fmt.Errorf("duplicate sorting position: %d", cp.Sorting)
		}
		seen[cp.Sorting] = true
	}
	...
	// Validate all issues are actually in this column
	cards, _, err := project_model.GetProjectCardsInColumn(ctx, column.ID, db.ListOptions{})
	if err != nil {
		return err
	}
	cardInColumn := make(map[int64]bool, len(cards))
	for _, card := range cards {
		cardInColumn[card.IssueID] = true
	}
	for _, cp := range cardPositions {
		if !cardInColumn[cp.IssueID] {
			return fmt.Errorf("issue %d is not in column %d", cp.IssueID, column.ID)
		}
	}
