...
func TestChangeProjectStatus(t *testing.T) {
	...
	t.Run("Change repository project status to closed", func(t *testing.T) {
		...
		err = ChangeProjectStatus(db.DefaultContext, reloadedProject, false)
		require.NoError(t, err)
	...
	t.Run("Change organization project status to closed and reopen", func(t *testing.T) {
		opts := CreateProjectOptions{
		...
		require.NoError(t, err)
		assert.True(t, reloadedProject.IsClosed)
		assert.NotZero(t, int64(reloadedProject.ClosedDateUnix))

		// Open the project again — ClosedDateUnix should be cleared
		err = ChangeProjectStatus(db.DefaultContext, reloadedProject, false)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		assert.False(t, reloadedProject.IsClosed)
		assert.Zero(t, int64(reloadedProject.ClosedDateUnix))
	})
}
