...
func TestDeleteColumn(t *testing.T) {
	...
	t.Run("No error when deleting non-existent column", func(t *testing.T) {
		// Create a fake column with non-existent ID
		fakeColumn := &project_model.Column{
			ID: 99999,
		}

		err := DeleteColumn(db.DefaultContext, fakeColumn)
