...
import (
	...
	permissions_project "forgejo.org/services/f3/permissions/project"
...
func (o *project) Get(ctx context.Context) bool {
	...
	f3_permissions.Check(ctx, permissions_project.Get(o.forgejoProject), "GET /repos/{username}/{reponame}")
...
func (o *project) Put(ctx context.Context) f3_id.NodeID {
	...
	if o.forked.IsNil() {
		if owner.IsOrganization() {
			f3_permissions.Check(ctx, permissions_project.PutOrganization(owner, o.forgejoProject), "POST /orgs/{org}/repos")
		} else {
			f3_permissions.Check(ctx, permissions_project.PutUser(o.forgejoProject), "POST /user/repos")
		}
		...
		f3_permissions.Check(ctx, permissions_project.PutFork(o.forgejoProject.BaseRepo, organizationName), "POST /repos/{username}/{reponame}/forks")
	...
	_, err = f3_resource_model.Upsert(ctx, f3_resource_model.NewResource(o.getForgejoMirrorID(ctx), o.forgejoProject.ID, f3_resource_model.KindRepository))
	if err != nil {
		panic(err)
	}
	o.setProjectAvatar(ctx)
...
func (o *project) Delete(ctx context.Context) {
	node := o.GetNode()
	o.Trace("%s", node.GetID())
	ownerID := f3_tree.GetOwnerID(node)
	owner, err := user_model.GetUserByID(ctx, ownerID)
	if err != nil {
		panic(fmt.Errorf("GetUserByID %v %w", ownerID, err))
	}
	f3_permissions.Check(ctx, permissions_project.Delete(owner, o.forgejoProject), "DELETE /repos/{username}/{reponame}")
