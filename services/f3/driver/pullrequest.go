...
func (o *pullRequest) ToFormat() f3.Interface {
	...
	assignees := make([]f3.Reference, 0, len(o.forgejoPullRequest.Assignees))
	for _, assignee := range o.forgejoPullRequest.Assignees {
		assignees = append(assignees, f3_tree.NewUserReference(f3_util.ToString(assignee.ID)))
	}

	labels := make([]f3.Reference, 0, len(o.forgejoPullRequest.Labels))
	for _, label := range o.forgejoPullRequest.Labels {
...
func (o *pullRequest) FromFormat(content f3.Interface) {
	...
	o.forgejoPullRequest.Labels = labels
}
...
func (o *pullRequest) Patch(ctx context.Context) {
	...
	o.Trace("id = %d, repo_id = %d, index = %d, assignees = %v", id, project, index, o.forgejoPullRequest.Assignees)
	if _, err := db.GetEngine(ctx).Where("`id` = ?", id).Cols("name", "content", "is_closed", "milestone_id", "is_locked", "updated").NoAutoTime().Update(o.forgejoPullRequest); err != nil {
	...
	updateIssueAssignees(ctx, id, o.forgejoPullRequest.Assignees)
	updateIssueLabels(ctx, id, o.forgejoPullRequest.Labels)
}
...
func (o *pullRequest) Put(ctx context.Context) f3_id.NodeID {
	...
	updateIssueAssignees(ctx, o.forgejoPullRequest.ID, o.forgejoPullRequest.Assignees)
	updateIssueLabels(ctx, o.forgejoPullRequest.ID, o.forgejoPullRequest.Labels)
