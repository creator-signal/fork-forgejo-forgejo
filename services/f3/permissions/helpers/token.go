...
func CreateAdminToken(ctx context.Context, scopeLevel auth_model.AccessTokenScopeLevel) (string, func(), error) {
	...
	user := admins[0]
	scope := ToScopeString(scopeLevel)
	token, err := CreateToken(ctx, user, scope)
	if err != nil {
		return "", nil, err
	}
