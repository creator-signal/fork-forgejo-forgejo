...
func transferLingeringLogs(ctx context.Context, opts actions_model.FindTaskOptions) error {
	total, err := db.Count[actions_model.ActionTask](ctx, opts)
	if err != nil {
		return err
	}
	if total <= 0 {
	...
	err = db.Iterate(ctx, opts.ToConds(), func(ctx context.Context, task *actions_model.ActionTask) error {
		if err := TransferLogs(ctx, task); err != nil {
			return err
		}
		log.Debug("processed task %d", task.ID)
		count++
		if count < transferLingeringLogsMax && count != int(total) {
		...
		if count >= transferLingeringLogsMax {
			return fmt.Errorf("stopped after processing %v tasks out of %v and will resume later", transferLingeringLogsMax, total)
		}
		return nil
	})
	if count >= transferLingeringLogsMax {
