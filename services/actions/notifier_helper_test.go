...
func TestActionsNotifier_ExpandReusableWorkflow(t *testing.T) {
	...
	defer test.MockVariableValue(&expandLocalReusableWorkflows,
		func(commit *git.Commit) LocalReusableWorkflowFetcher {
	...
	remoteReusableCalled := []*model.RemoteReusableWorkflowWithBaseURL{}
	...
	dw := &actions_module.DetectedWorkflow{
		Content: []byte("{ on: pull_request, jobs: { j1: { uses: \"./.forgejo/workflows/reusable-path.yml\" }, j2: { uses: \"https://example.com/some-org/some-repo/.forgejo/workflows/reusable-path.yml@main\" }} }"),
	}
	testActionsNotifierPullRequest(t, repo, pr, dw, webhook_module.HookEventPullRequestSync)
	...
	assert.Equal(t, &model.RemoteReusableWorkflowWithBaseURL{
