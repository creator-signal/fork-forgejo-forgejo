...
func tryHandleIncompleteMatrix(ctx context.Context, blockedJob *actions_model.ActionRunJob, jobsInRun []*actions_model.ActionRunJob) (bool, error) {
	...
	incompleteWith, _, _, err := blockedJob.IsIncompleteWith()
	if err != nil {
		return false, fmt.Errorf("job IsIncompleteWith: %w", err)
	}
...
// without actually executing it by dispatching it to a runner.  There's no job execution logic, but we need to update
// state of a few things -- particularly workflow outputs.
//
