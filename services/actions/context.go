...
func generateGiteaContextForRun(ctx context.Context, run *actions_model.ActionRun) (*model.GithubContext, error) {
	...
	refProtected, err := git_model.IsRefProtected(ctx, run.Repo.ID, git.RefName(ref))
	if err != nil {
		return nil, fmt.Errorf("could not determine whether ref %q of repository %d is protected: %w", ref, run.RepoID, err)
	}
	...
	gitContextObj := &model.GithubContext{
		...
		RefProtected:     refProtected,                             // boolean, true if branch protections are configured for the ref that triggered the workflow run.
...
func GenerateGiteaContext(ctx context.Context, run *actions_model.ActionRun, job *actions_model.ActionRunJob) (map[string]any, error) {
	gitContextObj, err := generateGiteaContextForRun(ctx, run)
	if err != nil {
		return map[string]any{}, fmt.Errorf("could not generate Gitea context for run %d: %w", run.ID, err)
	}
