...
func transferOwnership(ctx context.Context, doer *user_model.User, newOwnerName string, repo *repo_model.Repository) (err error) {
	...
	if err := repo_model.WatchRepo(ctx, doer.ID, repo.ID, repo_model.WatchNoneSelection); err != nil {
		return fmt.Errorf("watchRepo: %w", err)
	}

	// Remove watch for organization.
	if oldOwner.IsOrganization() {
		if err := repo_model.WatchRepo(ctx, oldOwner.ID, repo.ID, repo_model.WatchNoneSelection); err != nil {
