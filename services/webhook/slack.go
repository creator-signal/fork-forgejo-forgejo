...
func (s slackConvertor) Push(p *api.PushPayload) (SlackPayload, error) {
	...
	for i, commit := range p.Commits {
		attachmentText.WriteString(fmt.Sprintf("%s: %s - %s", SlackLinkFormatter(commit.URL, commit.ID[:7]), SlackShortTextFormatter(commit.Message), SlackNameFormatter(commit.Author.Name)))
