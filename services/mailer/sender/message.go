...
func (m *Message) ToMessage() *gomail.Msg {
	...
	_ = msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderFrom), addr.String())
	_ = msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderTo), m.To)
	if m.ReplyTo != "" {
		msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderReplyTo), m.ReplyTo)
	}
	...
	if setting.MailService.SubjectPrefix != "" {
		msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderSubject), setting.MailService.SubjectPrefix+" "+m.Subject)
	} else {
		msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderSubject), m.Subject)
	}
	...
	if len(msg.GetAddrHeader(gomail.AddrHeader(gomail.HeaderMessageID))) == 0 {
		msg.SetAddrHeader(gomail.AddrHeader(gomail.HeaderMessageID), m.generateAutoMessageID())
	}
